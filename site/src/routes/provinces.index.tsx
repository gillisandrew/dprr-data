...
export const Route = createFileRoute("/provinces/")({
  ...
  head: () => ({
    meta: [
      { title: "Provinciae — DPRR" },
      {
        name: "description",
        content:
          "Provinciae — territories, courts, and spheres of responsibility with recorded office holders",
      },
    ],
  }),
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <header className="rule-lead pb-3">
        <h1 className="font-heading text-3xl font-bold">Provinciae</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {provinces.length} provinciae with recorded office holders
        </p>
      </header>
      <ul className="mt-2">
        {provinces.map((p) => (
