...
# Specific predicate+value pairs to strip — software provenance annotations.
# hasExtraInfo is also used for real scholarly notes, so we only strip the
# values that are auto-generation markers.
STRIP_VALUES = {
    NamedNode(VOCAB + "hasExtraInfo"): frozenset({
        "Autogenerated by add_inverse",
        "Inferred Inverse",
        "Inferred",
    }),
}
